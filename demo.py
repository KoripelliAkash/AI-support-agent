...
def interactive_demo():
    ...
    while True:
        try:
            ...
            console.print("[dim]Analyzing and retrieving historical resolutions...[/dim]")
            pred = agent.predict(user_input)

            # Display results
            esc_color = "red" if pred.action == EscalationAction.ESCALATE_TO_HUMAN else "green"

            table = Table(title="🤖 Agent Response & Triage Decision", border_style="blue")
            table.add_column("Field", style="bold white", width=20)
            table.add_column("Output", style="white")

            table.add_row("Customer Query", user_input)
            table.add_row("Classified Intent", f"[bold cyan]{pred.intent.value}[/bold cyan]")
            table.add_row("Intent Confidence", f"{pred.intent_confidence * 100:.1f}%")
            table.add_row("Triage Action", f"[{esc_color}][bold]{pred.action.value}[/bold][/{esc_color}]")
            table.add_row("Escalation Reason", pred.escalation_reason)
            table.add_row("Drafted Reply", f"[italic green]\"{pred.draft_reply}\"[/italic green]")
