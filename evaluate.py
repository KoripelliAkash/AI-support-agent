...
def load_golden_set() -> List[Dict[str, Any]]:
    ...
    if not GOLDEN_SET_PATH.exists():
        console.print(
            "[yellow][!] Golden evaluation set not found. Please run scripts/01_extract_and_clean.py and scripts/02_build_golden_set.py[/yellow]"
        )
...
def evaluate_system(
    ...
    for idx, item in enumerate(golden_set):
        ...
        if (idx + 1) % 10 == 0 or idx == len(golden_set) - 1:
            console.print(
                f"  [dim]Evaluating item {idx+1}/{len(golden_set)}...[/dim]", end="\r"
            )
    ...
    avg_grounded = (
        np.mean([s.groundedness_score for s in judge_scores]) if judge_scores else 0.0
    )
    avg_tone = (
        np.mean([s.tone_empathy_score for s in judge_scores]) if judge_scores else 0.0
    )
    avg_actionable = (
        np.mean([s.actionability_score for s in judge_scores]) if judge_scores else 0.0
    )
    avg_judge_overall = (
        np.mean([s.overall_score for s in judge_scores]) if judge_scores else 0.0
    )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Apple Support Agent Evaluation Harness"
    )
    parser.add_argument(
        "--sample-size",
        type=int,
        default=30,
        help="Number of golden examples to evaluate (default: 30 for <2 min runs)",
    )
    parser.add_argument(
        "--all", action="store_true", help="Evaluate the full 180 golden examples"
    )
    ...
    console.print(
        f"[bold]Evaluating on {len(eval_set)} stratified examples (out of {len(full_golden_set)} total)[/bold]\n"
    )
    ...
    res_trivial = evaluate_system(
        "1. Trivial Baseline (Regex)",
        trivial,
        eval_set,
        run_judge=True,
        judge_sample_size=len(eval_set),
    )
    results.append(res_trivial)

    # 2. Run Simple Baseline (Zero-Shot)
    res_simple = evaluate_system(
    ...
    res_agent = evaluate_system(
        "3. Apple Support AI Agent (RAG + Triage)",
        agent,
        eval_set,
        run_judge=True,
        judge_sample_size=len(eval_set),
    ...
    table = Table(
        title="🏆 Benchmark Comparison: Headline Results", header_style="bold magenta"
    )
    ...
    for i, fail in enumerate(agent_failures, 1):
        ...
        console.print(
            f"  [green]Ground Truth:[/green] Intent='{fail['true_intent']}', Escalate={fail['true_escalate']}"
        )
        console.print(
            f"  [red]Predicted:[/red]    Intent='{fail['pred_intent']}', Escalate={fail['pred_escalate']}"
        )
